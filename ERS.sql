...
CREATE PROCEDURE PROC_INSERT_REQUEST (P_USER_ID IN ERS_USERS.USER_ID%TYPE, P_AMOUNT IN REIMBURSEMENT_REQUESTS.AMOUNT%TYPE, P_REQUEST_DATE IN REIMBURSEMENT_REQUESTS.REQUEST_DATE%TYPE, P_DESCIPTION IN REIMBURSEMENT_REQUESTS.DESCIPTION%TYPE)
AS BEGIN
  INSERT INTO REIMBURSEMENT_REQUESTS VALUES(SEQ_REQUEST_ID.NEXTVAL, P_USER_ID, P_AMOUNT, P_REQUEST_DATE, P_DESCIPTION, NULL);
END PROC_INSERT_REQUEST;
...
CREATE PROCEDURE PROC_COMPARE_USER_PASSWORD (P_USER_ID IN ERS_USERS.USER_ID%TYPE, P_USER_PASSWORD IN ERS_USERS.USER_PASSWORD%TYPE, O_PASSWORDS_EQUAL OUT INTEGER)
AS
...
BEGIN
  ...
  IF N_USER_PASSWORD = P_USER_PASSWORD THEN
    O_PASSWORDS_EQUAL := 1;
  ELSE
    O_PASSWORDS_EQUAL := 0;
  END IF;
...
CREATE PROCEDURE PROC_IS_MANAGER (P_USER_ID IN ERS_USERS.USER_ID%TYPE, O_IS_MANAGER OUT NUMBER)
AS BEGIN
  SELECT MANAGER INTO O_IS_MANAGER FROM ERS_USERS WHERE USER_ID = P_USER_ID;
END PROC_IS_MANAGER;

CREATE PROCEDURE PROC_SELECT_ALL_REQUESTS (O_CURSOR OUT SYS_REFCURSOR)
AS BEGIN
  OPEN O_CURSOR FOR SELECT REQUEST_ID, USER_ID, AMOUNT, REQUEST_DATE, DESCIPTION, APPROVED FROM REIMBURSEMENT_REQUESTS;
END PROC_SELECT_ALL_REQUESTS;

CREATE PROCEDURE PROC_SELECT_USERS_REQUESTS (P_USER_ID IN ERS_USERS.USER_ID%TYPE, O_CURSOR OUT SYS_REFCURSOR)
AS BEGIN
  OPEN O_CURSOR FOR SELECT REQUEST_ID, AMOUNT, REQUEST_DATE, DESCIPTION, APPROVED FROM REIMBURSEMENT_REQUESTS WHERE USER_ID = P_USER_ID;
END PROC_SELECT_USERS_REQUESTS;
